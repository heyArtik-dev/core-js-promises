...
function queuePromises(promises) {
  const processNext = (index, accumulatedString) => {
    if (index >= promises.length) {
      return Promise.resolve(accumulatedString);
    }

    return promises[index].then((value) => {
      const newAccumulatedString = accumulatedString + value;
      return processNext(index + 1, newAccumulatedString);
    });
  };
